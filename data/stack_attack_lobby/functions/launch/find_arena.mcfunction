...
execute as @e[type=area_effect_cloud,tag=sat_new_arena] run function stack_attack_arena:setup/register_new_arena

# set flag to track whether an arena was found
scoreboard players set found_arena sat_data 0

# look for an arena
execute as @e[type=area_effect_cloud,tag=sat_arena,scores={sat_arena_state=0},limit=1,sort=random] at @s run function stack_attack_arena:setup/prepare_entities
